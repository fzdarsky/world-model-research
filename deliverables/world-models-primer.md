...
| -------- | -------- | -------- | ------------ |
| Latent-space WM | Future states in embedding space | V-JEPA 2, DreamerV3 | ✓ Yes |
| Pixel-space WM | Future video frames | Cosmos, Genie 3 | ✓ Yes |
| Physics-informed WM | States with physics constraints | PhysicsNeMo | ✓ Yes |
| World Action Models | States + actions jointly | DreamZero | ✓ Yes |
...
**Latent-space models** like V-JEPA 2 and DreamerV3 predict in compressed representation spaces — faster and more efficient than pixel prediction, but harder to inspect. **Pixel-space models** like NVIDIA Cosmos and Google DeepMind's Genie 3 generate full video frames, making predictions visually interpretable but computationally expensive and sensitive to pixel-level noise that latent-space models filter out. **Physics-informed models** blend learned dynamics with known constraints (conservation laws, kinematic limits), useful when some physics is understood. **World Action Models** like DreamZero learn state transitions and action affordances together, enabling both prediction and policy learning in a unified framework.

In contrast, **Vision-Language-Action models** (VLAs) like π0, NVIDIA GR00T N1, or Google's Gemini Robotics map observations directly to robot actions without explicit world modeling — they're powerful policies, but lack the predictive simulation capability that defines world models. **Knowledge-based systems** encode rules manually rather than learning from data.

**Can LLMs become world models?** Google DeepMind CEO Demis Hassabis has stated the goal of extending Gemini into a world model. In practice, DeepMind's strategy is a portfolio, not a single architecture: Gemini provides language understanding and reasoning, Genie 3 generates interactive simulated environments (a pixel-space world model), and SIMA agents learn embodied skills within those environments. Hassabis explicitly distinguishes predicting the next state of a physical environment from predicting the next word, and notes that current video models are visually realistic but not yet "physics-grade" — they don't reliably follow Newton's laws. The convergence is at the system level: an LLM for reasoning, a world model for simulation, and an agent for action — not an LLM that itself learns physical dynamics.
...
| ------ | --------------- | ------- |
| Cosmos Reason | "World Foundation Model" | VLM — analyzes video, outputs text, no future prediction |
...
| Agentic "world models" | "World Model" for digital agents | Environment-specific simulators that learn one app's UI transitions, not transferable dynamics |
...
When selecting a world model approach, there's a fundamental tradeoff between inference speed and the ability to inspect what the model predicts. **Latent-space models** like JEPA and DreamerV3 achieve fast inference (100-500ms) by predicting future states in compressed embedding spaces — but these predictions aren't human-readable. You can't visualize what the model thinks will happen; you can only observe the downstream decisions it makes. **Pixel-space models** like Cosmos and Genie operate at the opposite extreme: they generate full video frames, making predictions directly inspectable, but at a computational cost — often 5000ms or more per prediction.

**World Action Models** like DreamZero-Flash represent an emerging middle ground, achieving both speed (~150ms) and interpretability by learning compact action-conditioned representations that can be decoded into visual predictions when needed. **Physics-informed models** occupy a variable position depending on the domain and how much computation the physics constraints require.
...
| WAMs               | Newest approach; DreamZero promising but unproven at scale                                     |
...
**Combined architectures** are emerging, though integration remains loose. Cosmos Predict uses Cosmos Reason as its text encoder for richer prompt understanding, but they don't run jointly at inference. Developers chain them in workflows: Reason generates prompts, Predict simulates, Reason evaluates results. Vision AI agents pair LLMs for task planning with world models for action simulation. The emerging pattern: human intent expressed in language → LLM interprets and decomposes goals → world model simulates candidate actions → LLM selects best option → execution in physical environment.
...
**Cosmos Reason clarified:** Despite the "World Foundation Model" marketing label, Cosmos Reason is a physically-aware VLM that analyzes video and outputs text reasoning. It can reason about potential hazards in current frames, but it cannot simulate futures or answer "what happens in 2 seconds if I continue forward?" on its own. For future prediction, users must chain it with Cosmos Predict.

<img src="diagrams/nvidia-ecosystem.png" alt="NVIDIA Physical AI Ecosystem" width="800">

**What's open:** Cosmos and GR00T N1 are Apache 2.0 licensed with open weights. PhysicsNeMo and Isaac ROS are open source. This enables on-premise deployment and fine-tuning for specialized domains. **What's proprietary:** Omniverse and Isaac Sim remain closed, requiring licenses and often NVIDIA hardware for full functionality.
...
| Horizon        | Examples                                                                                  |
| -------------- | ----------------------------------------------------------------------------------------- |
| Deployed today | Tesla FSD (millions of vehicles), BADAS 2.0 / Nexar, Cosmos (2M+ DLs)                     |
| Deploying 2026 | Wayve Tokyo robotaxi launch, GXO humanoid warehouse pilots                                |
...
**Funding signals indicate enterprise confidence:** AMI Labs raised $1.03B (JEPA commercialization), World Labs $1B (3D world generation), Periodic Labs $300M (materials discovery). These are not science experiments — investors are betting on near-term revenue from world model deployments in robotics, autonomous vehicles, drug discovery, and industrial automation. The question is no longer "will world models work?" but "which approaches scale fastest and where?"
...
**Cross-domain applicability:** This advantage isn't unique to healthcare. Any domain with high observation noise benefits from latent-space prediction: industrial vibration sensors (bearing wear detection), satellite imagery (cloud cover, atmospheric distortion), RF signals (WirelessJEPA for channel state prediction in telecommunications), and financial time series (market microstructure noise). The pattern holds: when raw observations contain artifacts that obscure underlying structure, predicting in embedding space filters noise naturally.
...
**NVIDIA's position — weak here:** Healthcare is not NVIDIA's strength. The leading work comes from academic labs (Stanford, MIT) and startups like AMI Labs. NVIDIA's ecosystem dominates synthetic data generation (Cosmos) and robotics (GR00T), where pixel-space generation and physics simulation provide clear value. Medical imaging requires different capabilities — sample efficiency, noise resilience, interpretability for clinicians — where JEPA's academic roots give it advantage but production deployments remain unproven at scale.
...
| WAM           | ~150ms     | Good (video backbone)   | Planned (GR00T N2)                                 |
| JEPA-planning | ~100-500ms | Good (temporal)         | Research (V-JEPA 2-AC), production (BADAS 2.0)     |

**Vision-Language-Action (VLA) models** map directly from camera images and language instructions to robot joint angles — no intermediate world model, no prediction of future states. They're fast because they skip simulation, achieving ~50ms inference on modern GPUs. But this speed comes at a cost: VLAs learn reactive control patterns without building internal physics representations. They know "when I see this gripper configuration and you say 'grasp', output these motor commands," but they can't answer "what happens if I grasp 2cm to the left?" That limits their ability to adapt to novel scenarios or recover from unexpected perturbations.

**World Action Models (WAMs)** like DreamZero and the planned GR00T N2 unify state prediction and action generation in a single architecture, typically using video foundation models as backbones. They learn joint representations of observations and actions, enabling them to simulate future states *while* predicting optimal actions. Inference runs at ~150ms because the model must forward-propagate through both the world model and the policy. This is too slow for 30 Hz control loops but feasible for 7-10 Hz manipulation tasks where planning matters more than reaction speed.
...
1. **Pure learned world models** require large datasets but discover patterns implicitly. DeepMind's GNoME (Graph Networks for Materials Exploration) predicted 2.2 million stable crystal structures by learning formation energy landscapes from existing materials databases — no physics equations encoded, just graph neural networks trained on atomic arrangements and their thermodynamic stability. The model identified 528 new lithium-ion conductors, 39 of which have been experimentally validated. This approach excels when labeled data is abundant and physics is too complex to write down, but it demands millions of training examples to generalize reliably.
...
**When world models lose:** Leap71 Noyron proves that when physics is fully codifiable — thermodynamics, structural mechanics, fluid flow in constrained geometries — knowledge-based engineering outperforms learning. No training data, no hallucinations, no generalization risk. World models excel when dynamics are too complex to write as equations or when you need to *discover* rules from observations. But if you can write the rules, don't learn them.
